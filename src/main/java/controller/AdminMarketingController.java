...
public class AdminMarketingController extends HttpServlet {
    ...
    private void handlePromotionsGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int currentPage = 1;
        int pageSize = 10; // Default 10 promotions per page
        ...
        int totalPromotions = (allPromotions != null) ? allPromotions.size() : 0;
        ...
        int toIndex = Math.min(fromIndex + pageSize, totalPromotions);
        List<Promotion> promotions = (allPromotions != null && fromIndex < totalPromotions)
                ? allPromotions.subList(fromIndex, toIndex)
